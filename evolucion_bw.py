#!/usr/bin/python3
"""
evolucion_bw.py — Evolución de contratos Bigway (precinto ~* 'BWA').

Lee clientes BWA de naps_obras y registra su evolución contractual en records.
"""
...
from db import get_records_conn, get_gestionbw_conn, get_mesa_bw_conn
...
if not value:
    log.info("evolucion_bw: no hay clientes Bigway para procesar.")
    exit(0)

# ── 2. Insertar evolución contractual desde Gestion BW ────────────────────────
...
if rows:
    ...
    with conn_rec.cursor() as cur:
        if DRY_RUN:
            _print_dry_run_table("NUEVA EVOLUCIÓN CONTRACTUAL BIGWAY (evolucion_obras)", 
    ...
    log.info("evolucion_bw: %d registros de contrato insertados.", len(values))

# ── 3. Actualizar cambio de tecnología desde Mesa BW ─────────────────────────
conn_mesa = get_mesa_bw_conn()
...
if lista:
    ...
    with conn_rec.cursor() as cur:
        if DRY_RUN:
            _print_dry_run_table("ACTUALIZAR CAMBIO TECNOLOGIA MESA BIGWAY (evolucion_obras)", 
...
if updates:
    with conn_rec.cursor() as cur:
        if DRY_RUN:
            _print_dry_run_table("ACTUALIZAR MONTOS BIGWAY (evolucion_obras)", 
    ...
    log.info("evolucion_bw: %d montos actualizados.", len(updates))
